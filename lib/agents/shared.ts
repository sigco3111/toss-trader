...
${renderContextBlock(context)}`.trim();
}

function renderContextBlock(context?: MarketContext | null): string {
  if (!context) return "";

  const lines: string[] = [
    "",
    "## 실시간 시장 데이터 (수집 시각: " + context.collectedAt + ")",
    `- symbol: ${context.symbol}`,
    `- market: ${context.market}`,
  ];

  if (context.account) {
    lines.push(`- account: seq=${context.account.accountSeq}`);
  }

  const sections: Array<[string, unknown]> = [
    ["price (현재가)", context.price],
    ["orderbook (호가)", context.orderbook],
    ["recentTrades (체결)", context.recentTrades],
    ["priceLimits (상/하한가)", context.priceLimits],
    ["candles (1D × 20봉)", context.candles],
    ["stockInfo (종목 정보)", context.stockInfo],
    ["warnings (매수 유의)", context.warnings],
    ["marketCalendarKR (국내 장)", context.marketCalendarKR],
    ["exchangeRate (환율)", context.exchangeRate],
    ["holdings (보유)", context.holdings],
    ["buyingPower (매수 가능)", context.buyingPower],
    ["sellableQuantity (매도 가능)", context.sellableQuantity],
  ];
  ...
  if (context.errors.length > 0) {
    lines.push("");
    lines.push("### 부분 실패 (위 endpoint 결과는 신뢰하지 말 것)");
    lines.push("```json");
    lines.push(
      safeJson(
        context.errors.map((e) => ({ endpoint: e.endpoint, status: e.status, message: e.message })),
      ),
    );
    lines.push("```");
  }
